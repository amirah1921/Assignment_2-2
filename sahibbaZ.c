...
#define RULE 5 // Example: RULE is minimum 5 characters
...
int calculate_score(const char* word) {
    ...
    // Check the length of alphabetical characters
    ...
    // Check if the word meets the minimum character requirement
    if (alpha_count < RULE) {
        return 0;
    }

    // Iterate over each character in the word for scoring
    for (int i = 0; i < strlen(word); i++) {
        if (isalpha(word[i])) {
